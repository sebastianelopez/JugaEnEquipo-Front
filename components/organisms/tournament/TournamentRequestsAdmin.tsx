...
import { tournamentService, TournamentRequest } from "../../../services/tournament.service";
...
export const TournamentRequestsAdmin: FC<Props> = ({ tournamentId, onRequestUpdated }) => {
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const result = await tournamentService.findAllRequests(tournamentId);
      if (result.ok && result.data) {
        const pendingRequests = result.data.filter(
          (req: TournamentRequest) => !req.status || req.status === "pending"
        );
        ...
        for (const req of pendingRequests) {
          try {
            const teamResult = await teamService.find(req.teamId);
            if (teamResult.ok && teamResult.data) {
              teamsMap[req.teamId] = teamResult.data;
            }
          } catch (err) {
            console.error(`Error loading team ${req.teamId}:`, err);
          }
        }
        setTeams(teamsMap);
      }
  ...
  const handleReject = async (requestId: string) => {
    try {
      setProcessingIds((prev) => new Set(prev).add(requestId));
      const result = await tournamentService.rejectRequest(requestId);
      if (result.ok) {
        showSuccess({
      ...
      >
        ...
        <List sx={{ p: 0, width: "100%" }}>
          {requests.map((request) => {
            const team = teams[request.teamId] || {
              id: request.teamId,
              name: request.teamName || "Unknown Team",
            };
